...
export default function IlPastizzar() {
  ...
  useEffect(()=>{(async()=>{
    ...
    if(d){
      ...
      const cap = pets.kelb ? 16*3600 : 8*3600;
  ...
  useEffect(()=>{if(!loaded)return;const id=setInterval(()=>{
    ...
    if(hits>0){
      ...
      // award gelat for milestones (1 per milestone)
      for(let i=0;i<hits;i++) addGelat(1);
    ...
    if(festa.active && Date.now() < festa.endTime) {
      ...
      if (progressPct >= 100 && !festa.badge) {
        ...
        if (festa.reward.gelat) addGelat(festa.reward.gelat);
        if (festa.reward.badge) setFestaHistory(h => [...h, {date: new Date().toISOString(), event: festa.name, badge: festa.reward.badge}]);
        if (festa.reward.boost) setAdBoost({active:true, endsAt:Date.now() + festa.reward.boost.duration*1000});
  ...
  // ----- SUB-COMPONENTS (with new features) -----
  ...
  // ----- GDPR, Tutorial, Starter, Login, Leaderboard modals (unchanged) -----
